...
- Run `./scripts/copy_dot_env.sh` to copy the .env file from the templates. Note, the script may not exist, proceed either way.
...
- If `./app/server/.env` does not exist, instruct the user to fill out `./app/server/.env` based on `./app/server/.env.example`
