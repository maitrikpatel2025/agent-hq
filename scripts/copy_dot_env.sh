...
if [ -f "$PROJECT_ROOT/app/server/env.example" ]; then
    if [ ! -f "$PROJECT_ROOT/app/server/.env" ]; then
        ...
        echo -e "${YELLOW}app/server/.env already exists, skipping${NC}"
    fi
fi
...
if [ -f "$PROJECT_ROOT/app/client/env.example" ]; then
    if [ ! -f "$PROJECT_ROOT/app/client/.env" ]; then
        ...
        echo -e "${YELLOW}app/client/.env already exists, skipping${NC}"
    fi
else
...
echo ""
echo "Next steps:"
echo "  1. Edit app/server/.env with your credentials"
